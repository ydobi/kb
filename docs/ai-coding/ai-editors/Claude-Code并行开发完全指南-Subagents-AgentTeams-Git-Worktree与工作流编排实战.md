...
# Claude Code 并行开发完全指南：Subagents + Agent Teams + Git Worktree + 工作流编排实战
...
code-reviewer 负责审查，test-writer 负责写测试。两个可以并行工作。
```
...
当一个 Claude 实例处理不过来，需要真正同时运行多个 agent 时，Agent Teams 才是正确答案。
...
A：先安装 tmux。macOS 用户用 `brew install tmux`，Ubuntu/Debian 用 `sudo apt install tmux`。Windows 用户需要用 WSL2。
...
```text
architect（Agent Team 里）：用 /plan 分析 src/modules 里各模块的依赖关系，给出重构优先级建议。architect 的 plan 确认后，再分配给 frontend-dev 和 backend-dev 各自执行。

backend-dev：/plan
根据 architect 的分析结果，设计 database schema 迁移方案。backend-dev plan 确认后，执行迁移。
```
...
在项目根目录创建或编辑 `CLAUDE.md`，写入（示例结构，可按项目改写）：
...
# 在 Claude Code 里创建 Routine（命令与参数以官方文档为准）
