> 来源：[2026 年 AI 编码的「渐进式 Spec」实战指南](https://mp.weixin.qq.com/s/7Lgb3GfgXKI0J9L9e9sq0w)  
> 作者：逸驹（阿里云开发者）

# 2026 年 AI 编码的「渐进式 Spec」实战指南

> **阿里妹导读**：这次分享的内容来自作者在实际项目中落地 AI 编码的一些实践和思考。希望能给正在尝试或想要尝试 AI 编码的同学一些参考。
...
- **本质复杂度**由业务决定，任何工具都不能消除它——交易状态机该多复杂还是多复杂。
- **AI Coding 工具**能做的是帮你更高效地应对本质复杂度（快速理解代码、生成实现、发现风险），但工具自身也会引入偶然复杂度（学习成本、流程开销、配置负担）。
...
这三条铁律在经济上也是合理的（**Code is Cheap, Context is Expensive**）：把需求、约束、代码现状写进 Spec 作为高质量输入 → 输入增加但便宜 → AI 不用反复试错 → 输出大幅减少 → 对话轮次从 20 轮降到 3–5 轮 → 总成本反而更低，效果反而更好。
...
1. **讨论和命令混为一谈**  
   「帮我看看这个模块，顺便把 Bug 修了」——前半句是探索，后半句是指令。AI 会跳过探索直接改代码，然后改错。解决办法：一次只给一种意图（探索 / 决策 / 指令 / 审查），不要混着来。

2. **阶段产出搞混**  
   调研阶段要的是事实和风险，不是代码。AI 太勤快的时候要拉住它：「停，我现在不需要代码，先告诉我现状和风险。」

3. **自由度给反了**  
   这是最普遍的问题。正确的自由度曲线如下表：
...
- [Superpowers](https://github.com/obra/superpowers) — agentic skills 框架（HARD-GATE、两阶段 review、systematic-debugging）
...
以下为原文附录中的模板文件，已换行整理，可直接复制到项目中使用（应用名、包名等需按实际填充）。
...
### A.11 `changes/templates/test-spec.md` — 单测 Spec 模板
